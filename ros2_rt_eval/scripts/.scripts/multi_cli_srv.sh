...
if [ "$#" -ne 5 ]; then
    echo "Usage: $0  <vector_size> <number_of_executions> <number_of_clients_per_srv> <number_of_srv> <directory> "
    exit 1
fi
...
do
    ...
    do
    # Execute the ROS 2 client node, passing the Client_ID, vector_size, and directory
    ros2 run ros2_rt_eval rt_cli_node  $NEXE $VECTOR_SIZE $j $i $DIR &
